...
- **Waypoint** - деплой
- **Terraform** - пока побаловаться поставил, но потом легко будет заменить, если перееду на cloud
...
honorable mention: мне надо было написать ещё Makefile, чтобы не возиться с ручной загрузкой данных на ноду, но его польза не слишком велика
