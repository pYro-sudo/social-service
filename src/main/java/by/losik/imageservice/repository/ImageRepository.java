...
public interface ImageRepository extends R2dbcRepository<Image, Long> {
    ...
    @Query("SELECT * FROM gallery.images WHERE user_id = :userId ORDER BY uploadedAt DESC")
    Flux<Image> findByUserId(@NonNull Long userId);

    @Query("SELECT * FROM gallery.images WHERE description LIKE :description ORDER BY uploadedAt DESC")
    Flux<Image> findByDescriptionContaining(@NonNull String description);

    @Query("SELECT * FROM gallery.images WHERE uploadedAt > :date ORDER BY uploadedAt DESC")
    Flux<Image> findByUploadedAtAfter(@NonNull LocalDate date);

    @Query("SELECT * FROM gallery.images WHERE uploadedAt < :date ORDER BY uploadedAt DESC")
    Flux<Image> findByUploadedAtBefore(@NonNull LocalDate date);

    @Query("SELECT * FROM gallery.images WHERE uploadedAt BETWEEN :startDate AND :endDate ORDER BY uploadedAt DESC")
